...
public sealed class NonogramCommandTests : IClassFixture<CommandAppFixture>
{
    private readonly CommandAppFixture _commandAppFixture;

    public NonogramCommandTests(CommandAppFixture commandAppFixture)
    {
        _commandAppFixture = commandAppFixture;
        _commandAppFixture.App.SetDefaultCommand<NonogramCommand>();
    }

    [Fact]
    public async Task Should_ParseArguments()
    {
        const string url = "https://www.nonograms.ru/nonograms/i/80413";
        const NonogramAlgorithm algorithm = NonogramAlgorithm.AStar;
        ...
        NonogramSettings? settings = result.Settings as NonogramSettings;
    ...
    public async Task ShouldExit_Success()
    {
        ...
        CommandAppResult result = await _commandAppFixture.App.RunAsync(["https://www.nonograms.ru/nonograms/i/80413", "-H", "true"], cts.Token);
